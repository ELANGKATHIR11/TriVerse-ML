    ...
    const interval = setInterval(() => {
      setTrainingProgress((prev) => {
        if (prev >= 100) {
          ...
          setIsTraining(false);
          const mockMetricValue = parseFloat((0.925 + Math.random() * 0.05).toFixed(3));
          addExperiment({
            id: "exp-" + Math.random().toString(36).substring(2, 5),
            name: `Credit-${selectedModelType}-AutoTuned`,
            status: "Completed",
            metricValue: mockMetricValue,
            loss: parseFloat((0.08 + Math.random() * 0.06).toFixed(3)),
            modelType: selectedModelType,
            dataset: selectedDataset,
            durationSeconds: Math.floor(60 + Math.random() * 320),
            createdAt: new Date().toISOString()
          });
          // Mutate confusion matrix slightly for variety
          setMatrix({
            tp: Math.round(850 + Math.random() * 50),
            fn: Math.round(15 + Math.random() * 10),
            fp: Math.round(30 + Math.random() * 20),
            tn: Math.round(140 + Math.random() * 20)
          });
          return 100;
        }
        return prev + 10;
